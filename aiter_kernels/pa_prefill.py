...
if triton.__version__ >= "2.1.0":
    ...
    def context_attention_fwd(
        ...
        if SHOULD_LOG:
            ...
            print(f"b_start_loc shape: {b_start_loc.shape}")
            print(f"b_seq_len shape: {b_seq_len.shape}")
            print(f"v_cache: {v_cache.shape[3]}")
            print(f"k_cache: {k_cache.shape[4]}")
            print(f"o shape: {o.shape}")
            print(f"b_loc.stride(0): {b_loc.stride(0)}")
            print(f"b_loc.stride(1): {b_loc.stride(1)}")
            print(f"q.stride(0): {q.stride(0)}")
            print(f"q.stride(1): {q.stride(1)}")
            print(f"q.stride(2): {q.stride(2)}")
            print(f"k.stride(0): {k.stride(0)}")
            print(f"k.stride(1): {k.stride(1)}")
            print(f"k.stride(2): {k.stride(2)}")
            print(f"v.stride(0): {v.stride(0)}")
            print(f"v.stride(1): {v.stride(1)}")
            print(f"v.stride(2): {v.stride(2)}")
            print(f"o.stride(0): {o.stride(0)}")
            print(f"o.stride(1): {o.stride(1)}")
            print(f"o.stride(2): {o.stride(2)}")
            print(f"k_cache.stride(0): {k_cache.stride(0)}")
            print(f"k_cache.stride(1): {k_cache.stride(1)}")
            print(f"k_cache.stride(2): {k_cache.stride(2)}")
            print(f"k_cache.stride(3): {k_cache.stride(3)}")
            print(f"k_cache.stride(4): {k_cache.stride(4)}")
            print(f"v_cache.stride(0): {v_cache.stride(0)}")
            print(f"v_cache.stride(1): {v_cache.stride(1)}")
            print(f"v_cache.stride(2): {v_cache.stride(2)}")
            print(f"v_cache.stride(3): {v_cache.stride(3)}")
            print(f"num_queries_per_kv: {num_queries_per_kv}")

        compiled_kernel = _fwd_kernel[grid](
            q,
            k,
            v,
            k_cache,
        ...
        if SHOULD_LOG:
            src: triton.compiler.compiler.ASTSource = compiled_kernel.src
            print(dir(compiled_kernel.src.constants), compiled_kernel.src.constants)
