import os
import torch
from aiter_kernels.pa_prefill import context_attention_fwd
...
if __name__ == '__main__':
    ...
    block_tables = torch.full((batch_size, max_input_len // block_size), 65535, dtype=torch.int32, device="cuda")
    block_tables[0, 0] = 0
    seq_lens = torch.full((batch_size,), 0, device="cuda")
    seq_lens[0] = 8
    start_loc = torch.full((batch_size+1,), 8, device="cuda")
    start_loc[0] = 0
    sliding_window = 0

    print("Parameters for context_attention_fwd:")
