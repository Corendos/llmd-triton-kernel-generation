...
def unified_attention(
    ...
    if max_seqlen_q > 1 or total_num_q_blocks * num_kv_heads > 128:
        ...
        if SHOULD_LOG:
            ...
            print(f"query_stride_0: {q.stride(0)}")
            print(f"query_stride_1: {q.stride(1)}")
            print(f"output_stride_0: {out.stride(0)}")
            print(f"output_stride_1: {out.stride(1)}")
            print(f"stride_k_cache_0: {k.stride(0)}")
            print(f"stride_k_cache_1: {k.stride(1)}")
            print(f"stride_k_cache_2: {k.stride(2)}")
            print(f"stride_k_cache_3: {k.stride(3)}")
            print(f"stride_v_cache_0: {v.stride(0)}")
            print(f"stride_v_cache_1: {v.stride(1)}")
            print(f"stride_v_cache_2: {v.stride(2)}")
            print(f"stride_v_cache_3: {v.stride(3)}")
            print(f"softmax_scale: {softmax_scale}")
        compiled_kernel: triton.compiler.CompiledKernel = kernel_unified_attention_2d[grid](
            output_ptr=out,
            query_ptr=q,
            key_cache_ptr=k,
            value_cache_ptr=v,
        ...
        compiled_kernel: triton.compiler.CompiledKernel = kernel_unified_attention_3d[(
        ...
        if SHOULD_LOG:
            ast_src: triton.compiler.compiler.ASTSource = compiled_kernel.src
            print(f"kernel_paged_attention_3d: {ast_src.constants}")
            print(f"kernel_paged_attention_3d: {compiled_kernel.asm['ttir']}")

        compiled_kernel: triton.compiler.CompiledKernel = reduce_segments[(q.shape[0], num_query_heads)](
            output_ptr=out,
            segm_output_ptr=segm_output,
            segm_max_ptr=segm_max,
            segm_expsum_ptr=segm_expsum,
        ...
        if SHOULD_LOG:
            ast_src: triton.compiler.compiler.ASTSource = compiled_kernel.src
            print(f"reduce_segments: {ast_src.constants}")
            print(f"reduce_segments: {compiled_kernel.asm['ttir']}")
